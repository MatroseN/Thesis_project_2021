...
class Model_27:
    def __init__(self, random_seed, serial, seed_serial):
        # Settings for Activation function
        self.activation_function = 'hard_sigmoid'

        # Ordinary settings
        self.author = "Andreas"
        self.name = self.__class__.__name__
        self.description = self.activation_function
        self.serial = serial
        self.ss = seed_serial
        ...
        self.model.add(Conv2D(filters=6, kernel_size=(3, 3), activation=self.activation_function, input_shape=(32, 32, 3)))
        self.model.add(AvgPool2D())
        self.model.add(Conv2D(filters=16, kernel_size=(3, 3), activation=self.activation_function))
        self.model.add(AvgPool2D())
        self.model.add(Flatten())
        self.model.add(Dense(units=120, activation=self.activation_function))
        self.model.add(Dense(units=84, activation=self.activation_function))
