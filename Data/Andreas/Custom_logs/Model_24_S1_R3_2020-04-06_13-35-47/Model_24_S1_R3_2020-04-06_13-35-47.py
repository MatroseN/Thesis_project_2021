...
class Model_24:
    def __init__(self, random_seed, serial, seed_serial):
        ...
        self.model.add(
            Conv2D(filters=6, kernel_size=(3, 3), activation=tf.keras.layers.LeakyReLU(alpha=self.leaky_relu_alpha),
                   input_shape=(32, 32, 3)))
        self.model.add(AvgPool2D())
        self.model.add(
            Conv2D(filters=16, kernel_size=(3, 3), activation=tf.keras.layers.LeakyReLU(alpha=self.leaky_relu_alpha)))
        self.model.add(AvgPool2D())
